// <copyright file="MainWindow.xaml.cs" company="Karatekid2501">
...
{
    /// <summary>
    /// Type of method to affact a database file.
    /// </summary>
    public enum MainWindowLoadCreate
    {
...
#pragma warning restore SA1602 // Enumeration items should be documented
    ...
    public partial class MainWindow : AdvanceWindow
    {
            ...
            : base()
        {
            ...
            switch (reportFrequncy)
            {
                ...
                case "Monthly":
                    ...
                    break;
                default:
                    throw new ArgumentException($"{reportFrequncy} is not a frequency type!!!");
            }
        }
        ...
        public void ShowOverlay(Visibility v) => Overlay.Visibility = v;

        /// <summary>
        /// Creates/Loads/Backups databases.
        /// </summary>
        /// <param name="loc">Location of the database.</param>
        /// <param name="mwlc">Method to call.</param>
        /// <param name="extraInfo">Any extra information that is needed.</param>
        /// <returns>True if successful.</returns>
        /// <exception cref="ArgumentException">Throws when an unknown method enum is called.</exception>
        public bool CreateLoadDatabase(string loc, MainWindowLoadCreate mwlc, object? extraInfo)
        {
            return mwlc switch
            {
                MainWindowLoadCreate.CreateDatabase => _container.NewServer(loc),
                MainWindowLoadCreate.LoadDatabase => _container.ExistingServer(loc),
                MainWindowLoadCreate.BackupDatabase => _container.BackupDatabase(loc, (bool)extraInfo!),
                _ => throw new ArgumentException($"Unknow MainWindowLoadCreate value [{mwlc}] has no return value attached"),
            };
        }
        ...
        {
            if (MessageBox.Show(
                "Are you sure you want to clear the server, this action can not be undone?",
                "Clear Server",
        ...
        {
            if (MessageBox.Show(
                $"Are you sure you want to delete {_dataRowView![1]} from the server",
                $"Delete {_dataRowView![1]}",
                MessageBoxButton.YesNo) == MessageBoxResult.Yes)
            {
        ...
        private void Coin_List_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (_dataRowView != null)
            {
                ...
                Description_Textbox.Text = _dataRowView[2].ToString();
            ...
            else
            {
                ...
                Description_Textbox.Text = "No data for coin description";
        ...
        {
            if (sender is MenuItem menu && menu.Name.Contains("RFCF"))
            {
                CheckRFCF(menu.Name);
        ...
        {
            if (!RFCF_Daily.IsChecked && !RFCF_Weekly.IsChecked && !RFCF_Monthly.IsChecked)
            {
                RFCF_Daily.IsChecked = true;
            }
        }
        ...
        private void HelpPage(object sender, RoutedEventArgs e)
        {
            if (App.GetInstance().Report.ShowMessage(
                "Are you sure you want open the help menu. It will open a new webpage",
                "Info",
                ReportSeverity.Info,
        ...
        {
            if (App.GetInstance().Report.ShowMessage(
                "Are you sure you want to create a new server",
                "Warning",
        ...
        {
            if (App.GetInstance().Report.ShowMessage(
                "Are you sure you want to select another server",
                "Warning",
                ReportSeverity.Warning,
                MessageBoxButton.YesNo,
                MessageBoxImage.Warning) == MessageBoxResult.Yes)
